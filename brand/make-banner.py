...
<style>
  .pulse-bad {{ animation: bad 6s cubic-bezier(.4,0,.2,1) infinite; }}
  @keyframes bad {{ 0% {{ transform: translateX(0) }} 45%,100% {{ transform: translateX(300px) }} }}
  .pulse-good {{ animation: good 6s cubic-bezier(.4,0,.2,1) infinite; }}
  @keyframes good {{ 0%,50% {{ transform: translateX(0) }} 95%,100% {{ transform: translateX(632px) }} }}
  @media (prefers-reduced-motion: reduce) {{ .pulse-bad, .pulse-good {{ animation: none; opacity: 0 }} }}
</style>
...
<g font-size="13.5">
  ...
  <circle class="pulse-bad" cx="94" cy="69" r="5" fill="{warn}"/>
...
<g font-size="13.5">
  ...
  <circle class="pulse-good" cx="94" cy="187" r="5" fill="{lime2}"/>
